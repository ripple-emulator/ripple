...
function createAudio(id, src, error) {
    ...
    a.addEventListener("durationchange", function () {
        if (messageChannel) {
            // If the plugin has provided us with a message channel, use that so the code flow
            // is as similar as possible to an actual device.
            messageChannel({action: "status", status: {id: id, msgType: 2, value: this.duration}});
        } else {
...
module.exports = {
    ...
    messageChannel: function (success) {
        // If the plugin provides a message channel, use that for communication back to the plugin.
